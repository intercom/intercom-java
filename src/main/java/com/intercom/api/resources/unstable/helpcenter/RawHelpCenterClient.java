...
public class RawHelpCenterClient {
        ...
        try (Response response = client.newCall(okhttpRequest).execute()) {
            ...
            if (response.isSuccessful()) {
                return new IntercomHttpResponse<>(
                        ObjectMappers.JSON_MAPPER.readValue(responseBodyString, CollectionList.class), response);
            }
            try {
    ...
    public IntercomHttpResponse<HelpCenterList> listHelpCenters(RequestOptions requestOptions) {
        ...
        try (Response response = client.newCall(okhttpRequest).execute()) {
            ...
            if (response.isSuccessful()) {
                return new IntercomHttpResponse<>(
                        ObjectMappers.JSON_MAPPER.readValue(responseBodyString, HelpCenterList.class), response);
            }
            try {
                if (response.code() == 401) {
                    throw new UnauthorizedError(
